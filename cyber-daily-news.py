...
- ✅ If the feed "summary/content" contains images (or looks like an image-only snippet),
     we skip those images and keep only cleaned text.
  - Removes <img ...>, <figure ...>, <svg ...>, <picture ...>, and markdown images ![](...)
  - Also strips leftover "data:image/..." blobs.
"""
...
def strip_images_from_html(raw: str) -> str:
    """
    Remove common embedded image constructs from feed HTML/markdown snippets.
    """
    if not raw:
        return ""
    s = raw
...
def strip_html_to_text(raw: str) -> str:
    """
    Convert HTML to plain-ish text after removing images.
    """
    if not raw:
        return ""
    s = html.unescape(raw)
...
def extract_summary(entry) -> str:
    ...
    # ✅ Remove images, then strip to text
...
def pdf_public_url(pdf_path: str) -> str:
...
def main() -> None:
    ...
    if SEND_WHATSAPP_PDF:
        url = pdf_public_url(pdf_path)
        caption = f"Cybersecurity Briefing • Top {TOP_N} • {LOOKBACK_HOURS}h • PDF attached"
        twilio_send_message(caption, media_url=url)
        print(f"✅ WhatsApp PDF sent (Twilio fetched): {url}")

    if not SEND_WHATSAPP_TEXT and not SEND_WHATSAPP_PDF:
        print("ℹ️ Not sending via WhatsApp. Set SEND_WHATSAPP_TEXT=1 and/or SEND_WHATSAPP_PDF=1.")
